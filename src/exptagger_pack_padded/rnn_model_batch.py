...
class rnn_model(nn.Module):
    ...
    def forward(self, input,lens, hidden, batch_first=True):
        ...
        idx = torch.tensor(lens,dtype=torch.long)
        last_index_use = 1
        idx = (idx-last_index_use).view(-1, 1)
        idx = idx.expand(len(lens), output.size(2))
