    ...
    with torch.no_grad():
        for sentindex, sent in enumerate(sents_as_ids):
            ...
            try:
                ...
                print(f'ignoring: {sent} while testing')
                del y_test[sentindex]
                continue
    ...
    for e in range(1,n_epochs+1):
        ...
        for sent,label in XY:
            ...
            try:
                ...
                print(f'ignoring the sentence: {sent}, label: {label}')
                continue
...
def main(load_embedding=False, max_runs=5):
    # sents, labels = utils.load_imdb_dataset();n_classes=2
    sents, labels = utils.load_rotten_tomatoes_dataset();n_classes=5
    ...
    for run in range(1, max_runs+1):
        ...
        if load_embedding:
            model = train(train_sent_ids, y_train, embeddings, n_epochs=10, vocab_size=len(word_id_map),
                          lr=0.1, use_pretrained_wv=True, n_classes=n_classes)
        else:
            model = train(train_sent_ids, y_train, embeddings, n_epochs=10, vocab_size=len(word_id_map),
                          lr=0.1, use_pretrained_wv=False, n_classes=n_classes)
